...
- Отсутствие ORM-слоя: Krust не управляет состоянием объектов и не привязан к моделям – он напрямую генерирует SQL-запросы, предоставляя полный контроль над итоговым запросом и избегая накладных расходов ORM.
...
6. **Удаление записей**.
    ```rust
    use krust::expression::helpers::{col, table, val};
    use sqlx::FromRow;
