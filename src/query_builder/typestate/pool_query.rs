...
/// Контекст пула: здесь будут методы параллелизации (into_send/spawn)
#[derive(Clone)]
pub struct PoolQuery<'a, T = ()>(pub(crate) QueryBuilder<'a, T>);

// Удобные конструкторы (crate-visible)
impl<'a, T> PoolQuery<'a, T> {
    pub(crate) fn new(inner: QueryBuilder<'a, T>) -> Self {
        Self(inner)
    }
}

// Доступ к базовому билдeру (DSL остаётся прежним через Deref)
impl<'a, T> std::ops::Deref for PoolQuery<'a, T> {
    type Target = QueryBuilder<'a, T>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl<'a, T> std::ops::DerefMut for PoolQuery<'a, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

// === Public interface for PoolQuery ===
impl<'a, T> PoolQuery<'a, T> {
    // ALIAS
    ...
    // На будущее: чтобы не упереться снова в те же грабли
    #[inline]
